...
# RedisAI Client
redisai_client = Client(host='redis-master-1', port=6379)

# current file path
FILE_PATH = Path(__file__).parent
# model output path
MODEL_OUTPUT_PATH = Path.joinpath(FILE_PATH, '_output/iris.onnx')
...
async def train():
    ...
    save_sklearn(model, MODEL_OUTPUT_PATH,
                 prototype=dummy)  # save model to MODEL_OUTPUT_PATH
...
@app.get('/inference')
async def inference(model: str, sepal_length: float, sepal_width: float,
    ...
    if model_meta:
        ...
        # 3. Predict
        redisai_client.modelexecute(model,
                                    inputs=[f'{model}:in'],
                                    outputs=[f'{model}:out1', f'{model}:out2'])
